exports.countSyllables = function (inString) {
    ...
    if (wordList) {wordList.forEach((word) => {
        ...
        const esylm = word.match(/ely|some([^aeiouy]|$)|ere(?!d|$)|(((?!c[hrl]|sh|\w[iszx]|ag|bl).{2}|^.{0,1})es$)|([aeiouy][^aeiouyrdt]|[^aeiouy][^laeiouyrdt]|[aeiouy][^aeiouy][^aeiouyrdt]|ll)ed$|[^c]red$/gmi);

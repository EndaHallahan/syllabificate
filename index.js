exports.countSyllables = function (inString) {
    ...
    if (wordList) {wordList.forEach((word) => {
        ...
        const esylm = word.match(/ely|every|some([^aeiouy]|$)|ere(?!d|$)|(((?!c[hrl]|sh|\w[iszx]|ag|bl|\wg).{2}|^.{0,1})es$)|([aeiouy][^aeiouyrdt]|[^aeiouy][^laeiouyrdt]|[aeiouy][^aeiouy][^aeiouyrdt]|ll)ed$|[^cd]red$|ive(?!$|r)/gmi);
        ...
        const asylm = word.match(/aste(?!$)|ace(?!$)/gmi);

exports.countSyllables = function(inString) {
    ...
    if (wordList) {wordList.forEach((word) => {
        ...
        const isylp = word.match(/rie[^snd]|[^t]ia|[^ct]ious|ism(s$|$)|quie|lier|ttli|stia|aying|kier|rien[^d]|[aeioyu]ing|ithm|dei[tf]/gmi);
        if (isylp) {syllables += isylp.length;} //I clustered positive
        const esylp = word.match(/rea([^l])\1|eor|ie$|peo|nuine|cle$|able$|ttle$|(?!(^))dea|ywed$|phe$|ucle[aeiouy]|kle(s|$)|inges/gmi);
        if (esylp) {syllables += esylp.length;} //E clustered positive
        const esylm = word.match(/ely|some([^aeiouy]|$)|ere(?!d|$)|(((?!c[hrl]|sh|\w[iszx]).{2}|^.{0,1})es$)|([aeiouy][^aeiouyrdt]|[^aeiouy][^laeiouyrdt]|[aeiouy][^aeiouy][^aeiouyrdt]|ll)ed$|[^c]red$/gmi);
        ...
        const usylp = word.match(/uo|ua[^r]/gmi);
